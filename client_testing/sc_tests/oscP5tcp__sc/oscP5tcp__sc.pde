...
void setup() {
   ...
   */
  oscP5tcpClient = new OscP5(this, "127.0.0.1", 57120, OscP5.TCP);
}
...
void mousePressed() {
  ...
  oscP5tcpServer.send("/response", new String[]{"blahhh"});
}
...
void keyPressed() {
  ...
  for (TcpClient client : oscP5tcpServer.tcpServer().getClients()) {
    System.out.println("client ip: " + client.netAddress().address() + ", client port: " + client.netAddress().port());
  }
}

void oscEvent(OscMessage theMessage) {
  /* in this example, both the server and the client share this oscEvent method */
  System.out.println("### got a message " + theMessage.get(0).stringValue());
  if(theMessage.checkAddrPattern("/test")) {
    ...
    System.out.println("tcp connection: " + theMessage.tcpConnection());
