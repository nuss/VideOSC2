...
public class OscPlug {
    ...
    public void plug(final Object theObject, final String theMethodName, final String theAddrPattern) {
        ...
        if (_myMethodName != null && _myMethodName.length() > 0) {
            ...
            for (Method myMethod : myMethods) {
                if ((myMethod.getName()).equals(_myMethodName)) {
                    myParams = myMethod.getParameterTypes();
                    for (Class<?> myParam : myParams) {
                        typeTag.append(checkType(myParam.getName()));
                    }
                    _myTypetag = String.valueOf(typeTag);
                    break;
                }
            }
    ...
    private void makeMethod(final Class<?> theObjectsClass, final Class<?>[] theClass) {
        ...
        try {
            ...
            assert theObjecsSuperClass != null;
    ...
    private Class<?>[] getArgs(final String theArgs) {
        ...
        for (int i = 0; i < tLen; i++) {
            switch (tChar[i]) {
                case ('i'):
                    tClass[i] = isArray ? int[].class : int.class;
                    break;
                case ('S'):
                case ('s'):
                    tClass[i] = isArray ? String[].class : String.class;
                    break;
                case ('f'):
                    tClass[i] = isArray ? float[].class : float.class;
                    break;
                case ('d'):
